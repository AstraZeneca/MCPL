...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        if presudo_words is not None:
            presudo_list = presudo_words
        else:
            presudo_list = placeholder_strings
        for idx, placeholder_string in enumerate(presudo_list):
            # if placeholder_string not in presudo_words:
            #     continue
    ...
    def forward(
        ...
        # print(f'DEBUG-EMB-MANAGER: start replace emb from tokenized_text: {tokenized_text}\n')
        print(f'DEBUG-EMB-MANAGER: string_to_token_dict keys: {self.string_to_token_dict.keys()}')
        for placeholder_string, placeholder_token in self.string_to_token_dict.items():
            if placeholder_token.clone().cpu() not in tokenized_text.clone().cpu():
                continue
            print(f'DEBUG-EMB-MANAGER: replacing {placeholder_string}={placeholder_token} now ...')
